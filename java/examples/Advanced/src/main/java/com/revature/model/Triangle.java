...
public final class Triangle extends Shape {
	...
	/**
	 * 
	 * @throws NotRightTriangleException - if triangle is not right by Pythagoras per sides passed as parameters.
	 */
	public Triangle(Color color, Integer oppositeLeg, Integer adyacentLeg, Integer hypotenuse) {
		...
		if(!isRight(oppositeLeg, adyacentLeg, hypotenuse)) {
			//This is a runtime exception, so we don't have to duck nor catch it (even though we could)
			throw new NotRightTriangleException("The given triangle is not a Right one.");
		}
